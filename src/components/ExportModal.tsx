...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-fadeIn">
      ...
      {/* Modal Box */}
      <div 
        className="relative w-full max-w-2xl liquid-glass-elevated bg-white/95 backdrop-blur-2xl rounded-[32px] sm:rounded-[36px] border border-white/90 shadow-2xl p-6 sm:p-8 overflow-hidden z-10 max-h-[90vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="flex items-center justify-between pb-4 border-b border-[#132A86]/10 mb-5">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-[14px] bg-[#132A86]/10 flex items-center justify-center text-[#132A86]">
            ...
            <div>
              ...
              <p className="text-xs text-[#4A577D]">
                {lang === 'uz' ? '100% Bepul • Suv belgisisiz • Cheklovlarsiz' : '100% Free • No Watermark • Unrestricted Quality'}
              </p>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 text-[#4A577D] hover:text-[#0A143A] rounded-[12px] hover:bg-black/5 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 rounded-[18px] border text-left transition-all cursor-pointer flex flex-col justify-between ${
                      isSelected
                        ? 'border-[#132A86] bg-[#EEF6FF] ring-2 ring-[#132A86]/20'
                        : 'border-[#132A86]/10 bg-white/70 hover:bg-white'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-extrabold text-sm text-[#0A143A]">
                        {fmt.name}
                      </span>
                      <Icon className={`w-4 h-4 ${isSelected ? 'text-[#132A86]' : 'text-[#4A577D]'}`} />
                    </div>
                    <span className="text-[10px] text-[#4A577D]">
                      {fmt.desc}
                    </span>
                  </button>
                );
          ...
          <div>
            <div className="flex items-center justify-between mb-2.5">
              <label className="block text-xs font-bold text-[#132A86] uppercase tracking-wider">
                {lang === 'uz' ? '2. Ruxsat & Sifat (Quality)' : '2. Quality & Resolution'}
              </label>
              <span className="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">
                {lang === 'uz' ? 'Barcha sifatlar bepul' : 'All qualities are free'}
              </span>
            </div>
              ...
              {qualityPresets.map((q) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className={`text-[10px] font-mono mt-0.5 ${isSelected ? 'text-white/80' : 'text-[#4A577D]'}`}>
                      {q.res}px
                    </p>
        ...
        {/* Modal Footer CTA */}
        <div className="pt-5 border-t border-[#132A86]/10 mt-5 flex items-center gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-3 rounded-[16px] text-xs font-bold text-[#4A577D] hover:text-[#0A143A] hover:bg-black/5 transition-colors cursor-pointer"
          >
            {t.common.cancel}
          </button>
          ...
          <button
            ...
            className="apple-glass-cta flex-1 py-3.5 rounded-[18px] text-sm font-bold flex items-center justify-center gap-2 cursor-pointer shadow-lg disabled:opacity-70"
          >
            {isExporting ? (
              <>
              ...
              <>
                ...
                <span>
                  {format.toUpperCase()} — {lang === 'uz' ? 'Yuklab Olish' : 'Download Now'} ({resolution}px)
                </span>
              </>
            )}
          </button>
