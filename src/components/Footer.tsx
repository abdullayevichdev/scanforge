...
export const Footer: React.FC<FooterProps> = ({ onOpenLegal, onOpenAbout }) => {
  const { t, lang } = useLanguage();

  return (
    <footer className="mt-20 border-t border-[#132A86]/10 bg-white/75 backdrop-blur-2xl relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14 sm:py-18">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 items-start">
          
          {/* Brand & Creator Bio (4 cols) */}
          <div className="lg:col-span-4">
            <BrandLogo variant="standard" />
            <p className="text-xs sm:text-sm text-[#4A577D] mt-4 max-w-sm leading-relaxed">
              {t.footer.description}
            </p>

            <div className="mt-4 flex items-center gap-2 text-xs font-semibold text-[#132A86]">
              <ShieldCheck className="w-4 h-4 text-[#1FD0C2]" />
          ...
          {/* Specialized Generators (3 cols) */}
          <div className="lg:col-span-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-[#0A143A] mb-3.5">
              {t.footer.products || 'Products'}
            </h4>
            <ul className="space-y-2.5 text-xs text-[#4A577D]">
              <li>
                <a href="#scanner" className="hover:text-[#132A86] transition-colors flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#1FD0C2]" />
              ...
              <li>
                <a href="#dynamic-qr" className="hover:text-[#132A86] transition-colors flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#132A86]" />
